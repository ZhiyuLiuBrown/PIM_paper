...
Prior work (\cite{hsieh2016accelerating, Ahn2015:2}) has shown that 
...
Therefore, we use the performance of the flat-combining linked-list as an indicator 
...
\emph{Therefore, if the CPU takes more than 2X time to access memory than the PIM core, 
a PIM managed linked-list is still slower than a concurrent linked list serving parallel requests from only 
two CPU cores!}
If we assume $r_1 = 3$, as estimated by prior work, the throughput of the PIM-managed linked-list with combining should be 
...
The performance of the flat-combining linked-list is a good 
indicator of the performance of the PIM-managed linked-list, so we use it to estimate the throughput of the
of the PIM-managed algorithm, based on the assumption $r_1 = 3$. 
...
the paths we traverse through to reach them are distinct.
...
so their throughputs are roughly ${1 \over (\beta\latcpu)}$ and ${1 \over (\beta\latpim + \latmes)}$ respectively, 
