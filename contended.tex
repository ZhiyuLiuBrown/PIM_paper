...
in a concurrent data structure where \emph{many} threads try to access the same locations. 
...
state-of-the-art concurrent FIFO queues, such as the one using flat combining~\cite{Hendler10} 
...
The PIM core then pops out the node at the tail of the dequeue segment and 
...
and dequeue segments, when their previous attempts have failed due to changes in where those segments are located. 
...
Since changes in where enqueue and dequeue segments are located happen very infrequently, 
the overhead of such changes is negligible and therefore ignored to simplify our analysis.
(If the threshold of segment length in line 13 of enq(\textit{cid}, $u$) is a large integer $n$, 
...
we will focus on dequeues. The analysis of enqueues is almost identical. 

Assume there are $p$ concurrent dequeue requests by $p$ threads. 
Since each thread needs to perform a F\&A operation on a shared variable in the F\&A-based algorithm 
and F\&As on a shared variable are essentially serialized, 
...
Note that our analysis of the F\&A-based and the flat-combining algorithms is quite optimistic, 
as it counts only the costs in part of their executions. 
...
to be cached, the combiner has to perform a sequence of memory accesses to dequeue them one by one.  
...
After sending off the message containing the node in step 3, the PIM core \emph{immediately} retrieves the next 
...
The throughput of the PIM core is eventually decided by the costs of its memory accesses and local computations, 
as long as it has enough bandwidth to keep sending off messages,  
which is very likely the case for this algorithm, since only one message is sent per request.
More specifically, Figure \ref{figure:queue_pipeline}(b) illustrates that 
...
the total execution time of $p$ requests, 
including the time $\latmes$ spent by CPUs to their requests to the PIM core concurrently, 
